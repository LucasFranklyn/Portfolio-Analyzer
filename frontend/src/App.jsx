...
async function fetchLivePrices(tickers) {
    ...
    try {
        const res = await fetch(`http://localhost:8000/api/prices?tickers=${tickers.join(",")}`);
...
async function saveHolding(holding) {
    const res = await fetch("http://localhost:8000/api/holdings", {
        method: "POST",
...
async function deleteHolding(id) {
    await fetch(`http://localhost:8000/api/holdings/${id}`, { method: "DELETE" });
}
...
function useBlinkingCursor() {
    ...
    (() => { const t = setInterval(() => setShow(s => !s), 530); return () => clearInterval(t); }, []);
    return show ? "█" : " ";
}
...
// Light terminal palette — warm paper/parchment theme
const A = "#3d2e1a";   // dark brown — primary text
const BRIGHT = "#1a1008";   // near black — key values
const DIM = "#6b5540";   // mid brown — secondary text
const GHOST = "#a08060";   // tan — placeholders, minor labels
const GREEN = "#2d6e45";   // deep green
const RED = "#8b2e2e";   // deep red
const CYAN = "#1e5a7a";   // deep steel blue
const BG = "#f5f0e8";   // warm off-white background
const PANEL = "#ede8df";   // slightly darker panel
const BORDER = "#c8bfaa";   // warm tan border
const TOPBAR = "#ddd5c4";   // warm beige for bars/sidebar
const ACTIVE = "#c4b89e";   // slightly darker for active nav item
...
useEffect(() => {
  loadHoldings().then(data => setHoldings(data));
...
useEffect(() => {
  ...
  fetch("http://localhost:8000/api/settings", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ goal, weeklyDeposit }),
  });
    ...
    async function addHolding() {
        ...
        setAddingPrice(true);
        const saved = await saveHolding({ ticker, shares: parseFloat(newShares), type: newType });
    ...
    const inpStyle = (name) => ({
        ...
        border: "none",
        borderBottom: `1px solid ${inputFocus === name ? A : BORDER}`,
        color: BRIGHT,
    ...
    return (
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input, select { caret-color: ${A}; }
        select option { background: ${PANEL}; color: ${A}; }
        .trow:hover td { background: ${BORDER} !important; cursor: default; }
        @keyframes flicker { 0%, 100% { opacity: 1; } 95% { opacity: 0.99; } }
        .screen { animation: flicker 12s infinite; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .panel { animation: fadeIn 0.15s ease; }
        .fnkey { transition: all 0.05s; }
        .fnkey:hover { background: ${BORDER} !important; color: ${BRIGHT} !important; cursor: pointer; }
        .fnkey:active { background: ${GHOST} !important; }
        .navitem { transition: all 0.05s; }
        .navitem:hover { background: ${ACTIVE} !important; cursor: pointer; }
      `}</style>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "176px 1fr", minHeight: "calc(100vh - 100px)" }}>
                    ...
                    <div style={{ background: TOPBAR, borderRight: `1px solid ${BORDER}` }}>
                        ...
                        {tabs.map((t, i) => (
                            <div key={i} className="navitem" onClick={() => setTab(i + 1)}
                                style={{
                                    padding: "8px 10px", fontSize: "12px", letterSpacing: "0.04em",
                                    borderBottom: `1px solid ${BORDER}`,
                                    background: tab === i + 1 ? ACTIVE : "transparent",
                                    color: tab === i + 1 ? BRIGHT : DIM,
                                    borderLeft: tab === i + 1 ? `3px solid ${A}` : `3px solid transparent`,
                                }}>
                                {t}
                            </div>
                        ))}
                        ...
                        <div style={{ borderTop: `1px solid ${BORDER}`, marginTop: "8px", padding: "8px 10px" }}>
                            ...
                            {[
                                ...
                                ["MKT VALUE", loaded ? `$${analysis.totalValue.toLocaleString("en-US", { maximumFractionDigits: 0 })}` : "N/A"],
                                ["ETF WT", loaded ? `${Math.round(analysis.etfPct * 100)}%` : "N/A"],
                    ...
                    <div style={{ background: BG }}>
                        <div style={{ background: TOPBAR, borderBottom: `1px solid ${BORDER}`, padding: "3px 14px", display: "flex", justifyContent: "space-between", fontSize: "11px", letterSpacing: "0.08em" }}>
                            ...
                            <span style={{ color: GHOST }}>SIDEBAR TO NAVIGATE  |  F9=REFRESH PRICES  |  F4=ADD HOLDING</span>
                        </div>
                            ...
                            {tab === 1 && (
                                <div>
                                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1px", background: BORDER, border: `1px solid ${BORDER}`, marginBottom: "12px" }}>
                                        {[
                                            ["MKT VALUE", loaded ? `$${analysis.totalValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "N/A", BRIGHT],
                                            ["PROJ 1YR", loaded ? `$${analysis.projected.toLocaleString("en-US", { maximumFractionDigits: 0 })}` : "N/A", CYAN],
                                            ["ETF WEIGHT", loaded ? `${Math.round(analysis.etfPct * 100)}%` : "N/A", A],
                                            ["WKY DEPOSIT", `$${weeklyDeposit}`, GREEN],
                                        ].map(([k, v, c]) => (
                                            <div key={k} style={{ background: PANEL, padding: "8px 12px" }}>
                                                ...
                                                <div style={{ fontSize: "18px", color: c, letterSpacing: "0.04em" }}>{v}</div>
                                            </div>
                                        ))}
                                    </div>
                                            ...
                                            <thead>
                                                <tr style={{ background: TOPBAR, borderBottom: `1px solid ${BORDER}` }}>
                                                    {["TICKER", "TYPE", "SHARES", "CUR PX", "MKT VALUE", "PORT WT"].map(h => (
                                                        <th key={h} style={{ padding: "5px 10px", color: DIM, fontWeight: "normal", textAlign: "right", letterSpacing: "0.06em", fontSize: "10px", whiteSpace: "nowrap" }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                                    ...
                                                    return (
                                                        <tr key={h.id} className="trow" style={{ borderBottom: `1px solid ${BORDER}` }}>
                                                            <td style={{ padding: "6px 10px", color: BRIGHT, fontWeight: "bold", letterSpacing: "0.08em", textAlign: "left" }}>
                                                                {h.ticker}
                                                                <button onClick={() => { deleteHolding(h.id); setHoldings(p => p.filter(x => x.id !== h.id)); }}
                                                                    style={{ background: "none", border: "none", color: GHOST, cursor: "pointer", fontSize: "10px", marginLeft: "8px", fontFamily: "monospace", transition: "color 0.1s" }}
                                                                    onMouseEnter={e => e.target.style.color = RED}
                                                                    onMouseLeave={e => e.target.style.color = GHOST}>DEL</button>
                                                            </td>
                                                ...
                                                {!holdings.length && (
                                                    <tr><td colSpan={6} style={{ padding: "20px", textAlign: "center", color: GHOST, fontSize: "12px" }}>NO ACTIVE POSITIONS  //  USE TAB 4 TO ADD HOLDINGS</td></tr>
                                                )}
                                    ...
                                    {!loaded && <div style={{ fontSize: "10px", color: GHOST, padding: "4px 2px" }}>* CLICK F9 REFRESH OR USE SIDEBAR BUTTON TO LOAD LIVE PRICES FROM FINNHUB</div>}
                                </div>
                            )}
                                ...
                                <div>
                                    ...
                                    <div style={{ border: `1px solid ${BORDER}`, background: PANEL, padding: "12px", marginBottom: "12px" }}>
                                        ...
                                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "14px" }}>
                                            {[
                                                ["CURRENT MKT VAL", loaded ? `$${analysis.totalValue.toFixed(2)}` : "N/A", A],
                                                ["PROJECTED (1YR)", loaded ? `$${analysis.projected.toFixed(2)}` : "N/A", CYAN],
                                                ["TARGET", `$${goal.toLocaleString()}`, DIM],
                                            ].map(([k, v, c]) => (
                                                <div key={k}>
                                        ...
                                        <div style={{ marginTop: "8px", fontSize: "11px", color: DIM }}>
                                            {loaded && weeklyDeposit > 0
                                                ? `>> AT $${weeklyDeposit}/WK — EST. GOAL COMPLETION: ~${goal - analysis.totalValue <= 0 ? "0" : Math.ceil((goal - analysis.totalValue) / (weeklyDeposit * 4.33))} MONTHS`
                                                : ">> LOAD PRICES TO CALCULATE TIMELINE"}
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <div style={{ border: `1px solid ${BORDER}`, background: PANEL, padding: "12px", marginBottom: "12px", display: "flex", gap: "20px", alignItems: "flex-start" }}>
                                        ...
                                        <div>
                                            ...
                                            <div style={{ fontSize: "12px", color: A, lineHeight: 1.9 }}>
                                                {analysis.score >= 8
                                                    ? ">> PORTFOLIO WELL-STRUCTURED FOR SHORT-TERM BALANCED MANDATE.\n>> MAINTAIN DEPOSIT SCHEDULE. NO MATERIAL REALLOCATION REQUIRED."
                                                    : analysis.score >= 6
                                                        ? ">> ADEQUATE FOUNDATION. TARGETED ADJUSTMENTS COULD IMPROVE RISK-ADJ RETURNS.\n>> REVIEW RECOMMENDATIONS BELOW."
                                                        : analysis.score >= 4
                                                            ? ">> PORTFOLIO IN DEVELOPMENT PHASE. PRIORITIZE RECOMMENDATIONS BELOW.\n>> FOCUS ON DEPOSIT CONSISTENCY AND DIVERSIFICATION."
                                    ...
                                    {recs.map((rec, i) => (
                                        <div key={i} style={{
                                            ...
                                            borderLeft: `3px solid ${rec.priority === "!!!" ? RED : rec.priority === ">> " ? DIM : GHOST}`,
                                            background: PANEL, padding: "10px 12px", marginBottom: "6px",
                                        }}>
                                ...
                                <div>
                                    <div style={{ border: `1px solid ${BORDER}`, background: PANEL, padding: "16px", maxWidth: "500px" }}>
                                        ...
                                        <div style={{ display: "grid", gap: "14px" }}>
                                            {[
                                                ["TICKER SYMBOL", newTicker, setNewTicker, "ticker", "E.G. VTI", "text"],
                                                ["SHARE QUANTITY", newShares, setNewShares, "shares", "E.G. 10", "number"],
                                            ].map(([label, val, set, name, ph, type]) => (
                                                <div key={name}>
                                            ...
                                            <div style={{ fontSize: "10px", color: GHOST, lineHeight: 1.7 }}>
                                                * CURRENT PRICE FETCHED AUTOMATICALLY VIA FINNHUB ON SUBMIT
                                            </div>
                ...
                <div style={{ background: TOPBAR, borderTop: `1px solid ${BORDER}`, padding: "3px 6px", display: "flex", gap: "2px", fontSize: "11px" }}>
                    {[["F1", "HELP"], ["F2", "EDIT"], ["F3", "FIND"], ["F4", "ADD POS"], ["F5", "MENU"], ["F6", "PRINT"], ["F7", "BACK"], ["F8", "FWRD"], ["F9", "REFR"], ["F10", "EXIT"]].map(([fn, label]) => (
                        <div key={fn} className="fnkey"
                            ...
                            style={{ display: "flex", gap: "2px", padding: "2px 6px", cursor: "pointer", borderRadius: "1px" }}>
